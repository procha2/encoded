...
def main():
    ...
    conf_dir = main_args.conf_dir if main_args.conf_dir else DEFAULT_CONF_DIR
    # Load repo default config
    default_conf_path = f"{conf_dir}/dev-config.ini"
    conf_dict= _load_configuration(default_conf_path)
    # Load developer overrides
    override_conf_path = f"{conf_dir}/.dev-config.ini"
    conf_dict.update(_load_configuration(override_conf_path))
    # Main args override with deployment conf dict
    for key, val in conf_dict['deployment'].items():
