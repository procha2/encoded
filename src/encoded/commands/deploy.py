...
DEFAULT_CONF_DIR = f"{str(Path().parent.absolute())}"
...
def main():
    ...
    # Load deployment config inis
    conf_dict= _load_configuration(f"{DEFAULT_CONF_DIR}/demo-config.ini")
    conf_dict.update(_load_configuration(f"{DEFAULT_CONF_DIR}/.dev-config.ini"))

    # User name
    if not conf_dict['deployment']['username']:
        conf_dict['deployment']['username'] = getuser()
    # Branch
    if main_args.branch:
        branch = main.args.branch 
    elif conf_dict['deployment']['branch']:
        branch = conf_dict['deployment']['branch']
    else:
    ...
    # Ssh Key
    ssh_key_path = expanduser(conf_dict['deployment']['identitiy_file'])
    ssh_key = _read_ssh_key(ssh_key_path)
    if not ssh_key:
        print('WARNING: Could not locate local ssh public key')

    # System Args
    system_args = conf_dict['system']
    ...
    template_path = "{}/{}.yml".format(conf_dict['deployment']['conf_dir'], 'demo')
    demo_template = None
    ...
    tags = [
        ...
        for key, val in {
                ...
                'Role': 'encd-demo',
                'section': 'app-dev',
                'started_by': conf_dict['deployment']['username'],
            }.items()
    ]
